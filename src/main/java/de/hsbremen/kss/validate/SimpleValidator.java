            ...
            for (final Tour tour : plan.getTours()) {
                ...
                for (int i = 0; i < actions.size(); i++) {
                    ...
                    if (!action.timewindow().between(time)) {
                        SimpleValidator.LOG.warn("Tour #" + tour.getId() + ": action " + action + " not performed in time window: "
                                + action.timewindow() + " ; time: " + TimeUtils.convertToClockString(time));
                    }
                    ...
                    if (action instanceof FromDepotAction) {
                        ...
                        if (i > 0) {
                            SimpleValidator.LOG.warn("Tour #" + tour.getId() + ": " + fromDepotAction + " is not the first action, it is action no: "
                                    + i);
                            allRight = false;
                        }
                    ...
                    if (action instanceof ToDepotAction) {
                        ...
                        if (i - 1 == actions.size()) {
                            SimpleValidator.LOG.warn("Tour #" + tour.getId() + ": " + toDepotAction + " is not the last action, it is action no: "
                                    + i);
                            allRight = false;
                        }
                    ...
                    if (action instanceof OrderLoadAction) {
                        ...
                        if (amount > vehicle.getCapacity()) {
                            SimpleValidator.LOG.warn("Tour #" + tour.getId() + ": " + "vehicle " + vehicle + " overloaded (amount: " + amount
                                    + ", max: " + vehicle.getCapacity() + ") on " + orderLoadAction);
                            allRight = false;
                        }
